...
<%@include file="headerWithoutSidebar.jsp"%>
<style type="text/css">
...
<div id="print" class="container" style="margin-top:4%">
	<div class="row">
		<h3>
				...
				<tbody id="carttoorder">
					...
					<c:forEach items="${orderDetailList}" var="item">
						<tr style="vertical-align: baseline;">
							<td scope="row">
								<a  href="${pageContext.request.contextPath}/jewel/adminJewelDetail?styleno=${item.styleno }" 
							...
							<td ><span
								class="label label-primary">${item.jewelnum }件</span>（库存：${item.jewel.storage}件）</td>
							<td ><span
	...
	<div id="black"></div>
	<input type="button"  onclick=" a()" value="打印" class="btn btn-primary" style="margin-left: 40%"/>
...
function onPageChange(newPageValue) {
    ...
    //alert(newPageValue);
    window.location.href="${pageContext.request.contextPath}/jewel/adminOrderDetail?orderid=${orderid}&companyid=${companyid}&pagenow="+current_page+"&pagesize="+pagesize;
}  */
